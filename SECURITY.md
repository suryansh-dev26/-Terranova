...
Last updated: 2026-07 (anonymous-auth migration).

## Identity

Every install signs in with **Firebase Anonymous Auth** on first launch
(`firebase.js → ensureSignedIn()`). The auth `uid` is the canonical user id:
all `runs` and `territories` docs are keyed by it, and Firestore rules enforce
that a client can only write docs stamped with its own uid.

The friendly `Runner-XXXX` name is now purely cosmetic. It lives in
`users/{uid}.displayName` and is denormalized onto new run/territory docs for
cheap display (leaderboard, domination toasts).
...
- `users`: only `displayName` (1–50 chars), `createdAt`, `pushToken`.
